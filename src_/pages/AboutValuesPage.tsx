// pages/AboutValuesPage.tsx — 100% paramétrable depuis WP via GET /about/values
...
const ICON_EMOJI: Record<string, string> = {
  Shield: '🛡️', Leaf: '🌿', Heart: '❤️', Star: '⭐', TrendingUp: '🚀', Smile: '😊', Award: '🏆', Lightbulb: '💡',
};

function colorToBg(color: string): string {
  const map: Record<string, string> = {
    '#357600': '#f0fdf4', '#3b82f6': '#eff6ff', '#22c55e': '#f0fdf4',
    '#eb700f': '#fff7ed', '#f59e0b': '#fffbeb', '#ec4899': '#fdf2f8', '#8b5cf6': '#f5f3ff',
  };
  return map[color] ?? '#f9fafb';
}

const DEFAULT_VALUES = [
  { Icon: Shield,     emoji: '🛡️', title: 'Sécurité avant tout',   desc: "Chaque visiteur repart sain et sauf. Matériel certifié CE, contrôlé quotidiennement.", details: ['Matériel certifié CE/EN', 'Contrôle technique chaque matin', "Moniteurs diplômés d'État", 'Formations premiers secours'], color: '#3b82f6', bg: '#eff6ff', num: '01' },
  { Icon: Leaf,       emoji: '🌿', title: 'Respect de la nature',  desc: "Nos parcs sont des espaces vivants. On construit avec la forêt, pas contre elle.",    details: ['Partenaire ONF officiel', 'Zéro produit chimique', 'Matériaux biosourcés', 'Plan de gestion biodiversité'], color: GREEN,    bg: '#f0fdf4', num: '02' },
  { Icon: Heart,      emoji: '❤️', title: 'Plaisir partagé',       desc: "L'aventure ne vaut rien si elle ne crée pas un lien. Chaque activité génère des souvenirs.", details: ['Activités multi-niveaux', 'Espaces familles dédiés', 'Photos souvenirs offertes', 'Programme fidélité'], color: '#ec4899', bg: '#fdf2f8', num: '03' },
  { Icon: Star,       emoji: '⭐', title: 'Excellence du service', desc: "98% de satisfaction. On relit chaque avis, on forme sans cesse nos équipes.",          details: ['NPS moyen > 70', 'Réponse aux avis sous 24h', 'Formation continue', 'Audit qualité semestriel'], color: '#f59e0b', bg: '#fffbeb', num: '04' },
  { Icon: TrendingUp, emoji: '🚀', title: 'Innovation continue',   desc: "On ne s'endort jamais sur nos lauriers. Chaque saison apporte de nouvelles activités.",   details: ['1 nouveauté minimum / saison', 'R&D matériel en interne', 'Tests utilisateurs', 'Veille internationale'], color: '#8b5cf6', bg: '#f5f3ff', num: '05' },
];
...
export function AboutValuesPage() {
  ...
  const values = data.values?.length > 0
    ? data.values.map((v, i) => {
        ...
        return {
          Icon:    ICON_MAP[iconName] ?? local.Icon,
          emoji:   ICON_EMOJI[iconName] ?? local.emoji,
          title:   v.title   ?? local.title,
          desc:    v.desc    ?? local.desc,
          details: v.details ?? local.details,
          color:   v.color   ?? local.color,
          bg:      colorToBg(v.color ?? '') || local.bg,
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <section className="relative pt-32 pb-24 overflow-hidden" style={{ background: `linear-gradient(135deg, ${GREEN} 0%, #2d6100 100%)` }}>
        <motion.div className="absolute -top-20 -right-20 w-96 h-96 rounded-full blur-3xl bg-white/10"
          animate={{ scale: [1, 1.2, 1] }} transition={{ duration: 8, repeat: Infinity }} />
        <motion.div className="absolute bottom-0 left-1/4 w-80 h-80 rounded-full blur-3xl" style={{ backgroundColor: ORANGE, opacity: 0.2 }}
        ...
        <div className="container mx-auto px-6 relative z-10">
          ...
          <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }} className="max-w-3xl">
            ...
            <h1 className="text-5xl md:text-7xl font-black text-white leading-none mb-5">
              {hero.h1 || 'Ce qui nous'}<br />
              <span style={{ color: ORANGE }}>{hero.h1accent || 'définit.'}</span>
            </h1>
            <p className="text-lg text-white/70 leading-relaxed max-w-xl">
              {hero.intro || "5 valeurs. Pas des mots sur un mur — des engagements concrets qu'on honore chaque jour, dans chaque parc."}
            </p>
          </motion.div>
        </div>
        ...
        <div className="container mx-auto px-6 relative z-10 max-w-4xl">
          <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-14">
            <h2 className="text-4xl md:text-5xl font-black text-white mb-3">
              {sections.manifestoTitle
                ? sections.manifestoTitle.replace(/\.$/, '') + '.'
                : <span>Ce qu'on <span style={{ color: ORANGE }}>croit.</span></span>}
            </h2>
            <p className="text-white/50">
              {sections.manifestoSubtitle || "Notre manifeste, des convictions qu'on ne négocie pas."}
            </p>
          </motion.div>
      ...
      <section className="py-12 bg-white border-t border-gray-100">
        <div className="container mx-auto px-6 flex items-center justify-between max-w-5xl">
          ...
          <Link to="/about/parcs" className="inline-flex items-center gap-2 px-6 py-3 rounded-2xl font-black text-sm text-white"
