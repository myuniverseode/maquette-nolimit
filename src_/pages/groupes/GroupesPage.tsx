...
import { useGroupsHub } from '../../hooks/useGroupsData';
...
// ─── Données fallback des catégories ──────────────────────────────────────────
...
// ─── Sous-composants avec props ───────────────────────────────────────────────

function HeroSection({ heroTitle, heroSubtitle }: { heroTitle?: string; heroSubtitle?: string }) {
      ...
      <motion.div style={{ opacity }} className="relative z-10 container mx-auto px-6 pb-20 w-full">
        ...
        >
          ...
          <h1 className="text-6xl md:text-8xl font-black text-white leading-[0.9] tracking-tight mb-6">
            {heroTitle ? heroTitle : (<>L'aventure<br /><span style={{ color: ORANGE }}>se vit</span><br />ensemble.</>)}
          </h1>

          <p className="text-white/70 text-xl max-w-2xl leading-relaxed mb-10">
            {heroSubtitle || "Team building, anniversaires, sorties scolaires, réunions de famille — nous créons des expériences sur mesure pour chaque groupe, chaque occasion."}
          </p>
...
function StatsBar({ stats }: { stats: typeof DEFAULT_STATS }) {
  return (
...
function CategoryGrid({ categories }: { categories: typeof GROUP_CATEGORIES }) {
  return (
...
function TestimonialsSection({ testimonials }: { testimonials: typeof DEFAULT_TESTIMONIALS }) {
  return (
...
export function GroupesPage() {
  const { data: grpData } = useGroupsHub();

  // Stats : données WP si dispo, sinon fallback local
  const stats = grpData.stats?.length > 0
    ? grpData.stats.map((s: any) => ({ val: s.value, label: s.label, icon: s.icon || '📊' }))
    : DEFAULT_STATS;

  // Testimonials : données WP si dispo
  const testimonials = grpData.testimonials?.length > 0
    ? grpData.testimonials.map((t: any) => ({
        text: t.text || t.quote || '',
        ...
        avatar: t.avatar || '👤',
        category: t.category || '',
        categoryColor: t.categoryColor || GREEN,
      }))
    : DEFAULT_TESTIMONIALS;

  // Categories : enrichir les données WP avec le design local
  const categories = grpData.categories?.length > 0
    ? grpData.categories.map((wpCat: any) => {
        const local = GROUP_CATEGORIES.find(c => c.id === wpCat.id) || GROUP_CATEGORIES[0];
        return {
          ...local,
          label: wpCat.label || local.label,
          description: wpCat.description || local.description,
          minGroup: wpCat.minGroup || local.minGroup,
          maxGroup: wpCat.maxGroup || local.maxGroup,
          slug: wpCat.link || local.slug,
        };
      })
    : GROUP_CATEGORIES;
  ...
  return (
    <div className="min-h-screen bg-white">
      <HeroSection
        heroTitle={grpData.hero?.title}
        heroSubtitle={grpData.hero?.subtitle}
      />
