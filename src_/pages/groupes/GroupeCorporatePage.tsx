...
import { useGroupsCorporate } from '../../hooks/useGroupsData';
...
function ProgramTypesSection({ programTypes }: { programTypes: typeof PROGRAM_TYPES }) {
  return (
...
function SectorsSection({ sectors }: { sectors: typeof SECTORS }) {
  return (
    <section className="py-16 bg-white">
      <div className="container mx-auto px-6">
...
function TestimonialsSection({ testimonials }: { testimonials: typeof TESTIMONIALS_CORP }) {
  return (
...
function FAQSection({ faqItems }: { faqItems: typeof FAQ_CORP }) {
...
export function GroupeCorporatePage() {
  const { data: corpData } = useGroupsCorporate();

  const programTypes = corpData.programTypes?.length > 0
    ? corpData.programTypes.map((pt: any, i: number) => ({
        id: pt.id || `pt-${i}`,
        icon: pt.icon || PROGRAM_TYPES[i % PROGRAM_TYPES.length]?.icon || '🏢',
        title: pt.title || PROGRAM_TYPES[i % PROGRAM_TYPES.length]?.title || '',
        shortDesc: pt.desc || pt.shortDesc || PROGRAM_TYPES[i % PROGRAM_TYPES.length]?.shortDesc || '',
        color: pt.color || PROGRAM_TYPES[i % PROGRAM_TYPES.length]?.color || GREEN,
      }))
    : PROGRAM_TYPES;

  const programs = corpData.programs?.length > 0
    ? corpData.programs.map((p: any, i: number) => ({
        ...PROGRAMS[i % PROGRAMS.length],
        id: p.id || `prog-${i}`,
        icon: p.icon || PROGRAMS[i % PROGRAMS.length]?.icon || '🤝',
        title: p.title || PROGRAMS[i % PROGRAMS.length]?.title || '',
        duration: p.duration || PROGRAMS[i % PROGRAMS.length]?.duration || '',
        minPeople: p.minPeople || PROGRAMS[i % PROGRAMS.length]?.minPeople || 10,
        maxPeople: p.maxPeople || PROGRAMS[i % PROGRAMS.length]?.maxPeople || 100,
        price: p.price || PROGRAMS[i % PROGRAMS.length]?.price || '',
        description: p.description || PROGRAMS[i % PROGRAMS.length]?.description || '',
        includes: p.includes || PROGRAMS[i % PROGRAMS.length]?.includes || [],
        outcomes: p.outcomes || PROGRAMS[i % PROGRAMS.length]?.outcomes || [],
        popular: p.popular || PROGRAMS[i % PROGRAMS.length]?.popular || false,
        color: p.color || PROGRAMS[i % PROGRAMS.length]?.color || GREEN,
      }))
    : PROGRAMS;

  const sectors = corpData.sectors?.length > 0
    ? corpData.sectors.map((s: any) => ({ icon: s.icon || '🏢', label: s.label || s.name || '' }))
    : SECTORS;

  const testimonials = corpData.testimonials?.length > 0
    ? corpData.testimonials.map((t: any) => ({
        text: t.text || t.quote || '',
  ...
  return (
    <div className="min-h-screen bg-white">
      <CorporateHero />
      <ProgramTypesSection programTypes={programTypes} />
