// pages/AboutNewsPage.tsx — Actualités (connecté à l'API WordPress /news)
...
// Fallback articles hardcodés
const FALLBACK_ARTICLES = [
  { cat: 'Nouveauté', catColor: GREEN, date: '15 mars 2025', title: "Ouverture d'un 6ème parc à Toulouse", desc: 'On pose nos arbres dans le Sud ! Notre nouveau parc toulousain ouvre ses portes en avril 2025, avec 3 parcours inédits et une via ferrata de 800m.', reading: '3 min', big: true, emoji: '🎊', slug: '' },
  { cat: 'Innovation', catColor: '#8b5cf6', date: '1er mars 2025', title: 'Nouvelle via ferrata inaugurée à Nemours', desc: 'Un parcours de 1,2 km taillé dans la roche, avec 4 niveaux de difficulté. Ouverture officielle le 1er avril.', reading: '2 min', big: false, emoji: '🧗', slug: '' },
  { cat: 'Promo', catColor: ORANGE, date: "Jusqu'au 31 mars", title: '-20% sur les réservations groupe', desc: 'Ce mois-ci uniquement : -20% pour tout groupe de 8 personnes et plus. Valable sur tous nos parcs.', reading: '1 min', big: false, emoji: '🎁', slug: '' },
  { cat: 'Portrait', catColor: '#ec4899', date: '20 fév. 2025', title: 'Rencontrez Laura, notre cheffe monitrice à Chevry', desc: "De championne de grimpe à formatrice, Laura nous raconte comment elle a transformé sa passion en vocation.", reading: '5 min', big: false, emoji: '🌟', slug: '' },
  { cat: 'Événement', catColor: '#f59e0b', date: 'Déc. 2024', title: "Noël en forêt : le bilan d'une édition magique", desc: "3 500 visiteurs, des lumières dans les arbres, des ateliers créatifs pour enfants. Retour sur notre édition Noël.", reading: '4 min', big: false, emoji: '🎄', slug: '' },
  { cat: 'Innovation', catColor: '#8b5cf6', date: 'Oct. 2024', title: 'Parcours Black : le retour du plus difficile de France', desc: "Réouvert après refonte complète, le parcours Black de Nemours s'affirme comme le plus technique du réseau.", reading: '3 min', big: false, emoji: '🖤', slug: '' },
];

// Mapping catégories WP → couleur
...
function getCatColor(cat: string): string {
  return CAT_COLORS[cat] || GREEN;
}

export function AboutNewsPage() {
  const [filter, setFilter] = useState('Tout');
  const [articles, setArticles] = useState(FALLBACK_ARTICLES);
  const [categories, setCategories] = useState(['Tout', 'Nouveauté', 'Événement', 'Innovation', 'Promo', 'Portrait']);

  useEffect(() => {
    fetch(`${API_URL}/news?per_page=12`, {
      headers: { 'Content-Type': 'application/json', 'X-NoLimit-Key': API_KEY || '' },
    })
      .then(r => { if (!r.ok) throw new Error(`HTTP ${r.status}`); return r.json(); })
      .then((wpArticles: any[]) => {
        if (!Array.isArray(wpArticles) || wpArticles.length === 0) return;
        const mapped = wpArticles.map((a, i) => ({
          cat: a.category || 'Actualité',
          catColor: getCatColor(a.category || ''),
          date: a.date || '',
          title: a.title || '',
          desc: a.excerpt || '',
        ...
        setArticles(mapped);
        const cats = ['Tout', ...Array.from(new Set(mapped.map(a => a.cat)))];
        setCategories(cats);
      })
            ...
            {filtered.map((article, i) => (
              ...
              >
                <Link to={article.slug ? `/actualites/${article.slug}` : '/actualites'} className="flex flex-col flex-1">
                  ...
                  >
                    <div className="absolute inset-0 flex items-center justify-center text-7xl opacity-20">{article.emoji}</div>
                    <motion.div className="absolute inset-0 flex items-center justify-center text-7xl" whileHover={{ scale: 1.2 }} transition={{ type: 'spring' }}>
                      {article.emoji}
                    </motion.div>
