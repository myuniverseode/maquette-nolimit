...
const DEFAULT_ENTREPRISE_TESTIMONIALS = [
...
export function PourQuiEntreprisePage() {
  ...
  const formats = entData.formats?.length > 0 ? entData.formats : DEFAULT_FORMATS;
  const advantages = entData.advantages?.length > 0 ? entData.advantages : DEFAULT_ADVANTAGES;
  const clientLogos = entData.clientLogos?.length > 0 ? entData.clientLogos : DEFAULT_LOGOS;
  const testimonials = entData.testimonials?.length > 0 ? entData.testimonials : DEFAULT_ENTREPRISE_TESTIMONIALS;
