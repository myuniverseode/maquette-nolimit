// pages/AboutPage.tsx — Hub About, 100% paramétrable depuis WP
import { motion } from 'framer-motion';
import { History, Target, MapPin, Briefcase, Newspaper, Users, Clock, Heart, Award, Zap, ArrowRight } from 'lucide-react';
...
const ICON_MAP: Record<string, React.ElementType> = {
  History, Clock, Target, Heart, MapPin, Briefcase, Newspaper, Users, Award, Zap,
};
...
const DEFAULT_NAV_CARDS = [
  { to: '/about/histoire',    Icon: History,   emoji: '🏔️', title: 'Notre Histoire',  desc: "De Chevry à la France entière, 10 ans d'aventure collective.",            color: GREEN,  bg: '#f0f7e8' },
  { to: '/about/valeurs',     Icon: Target,    emoji: '💚', title: 'Notre ADN',       desc: 'Sécurité, nature, plaisir : les 5 piliers qui nous définissent.',         color: ORANGE, bg: '#fef3ea' },
  { to: '/about/parcs',       Icon: MapPin,    emoji: '📍', title: 'Nos Parcs',       desc: '5 parcs uniques, chacun avec son caractère et ses paysages.',             color: GREEN,  bg: '#f0f7e8' },
  { to: '/about/emplois',     Icon: Briefcase, emoji: '🧗', title: 'Nos Emplois',     desc: "Fais de ta passion ton métier. On recrute toute l'année.",                color: ORANGE, bg: '#fef3ea' },
  { to: '/about/actualites',  Icon: Newspaper, emoji: '🗞️', title: 'Actualités',     desc: 'Nouveautés, événements et coulisses du parc en temps réel.',              color: GREEN,  bg: '#f0f7e8' },
  { to: '/about/partenaires', Icon: Users,     emoji: '🤝', title: 'Partenaires',     desc: 'Les organisations qui partagent nos valeurs et notre vision.',             color: ORANGE, bg: '#fef3ea' },
];
...
export function AboutPage() {
  ...
  const navCards = data.navCards?.length > 0
    ? data.navCards.map((c, i) => {
        ...
        return {
          to:    c.link  ?? local.to,
          Icon:  ICON_MAP[c.icon] ?? local.Icon,
          emoji: (c as any).emoji ?? local.emoji,
          title: c.title ?? local.title,
          desc:  c.desc  ?? local.desc,
          color: c.color ?? ALT_COLOR(i),
          bg:    ALT_BG(i),
        };
      })
    : DEFAULT_NAV_CARDS;
            ...
            {navCards.map((card, i) => {
              ...
              return (
                <motion.div key={i} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
                  transition={{ delay: i * 0.08 }} whileHover={{ y: -8, scale: 1.01 }} className="group">
                  <Link to={card.to} className="block h-full">
                    <div className="h-full bg-white rounded-3xl p-7 shadow-sm border-2 border-gray-100 hover:border-gray-200 hover:shadow-xl transition-all flex flex-col gap-5">
                      <div className="flex items-start justify-between">
                        <div className="w-14 h-14 rounded-2xl flex items-center justify-center text-2xl" style={{ backgroundColor: card.bg }}>
                          {card.emoji}
                        </div>
                        <div className="w-9 h-9 rounded-xl flex items-center justify-center border-2 border-gray-100 group-hover:border-gray-300 transition-all mt-1">
                      ...
                      <div className="mt-auto pt-5 border-t border-gray-100">
                        <div className="h-1 w-10 rounded-full group-hover:w-20 transition-all duration-500" style={{ backgroundColor: card.color }} />
                      </div>
                    </div>
                  </Link>
                </motion.div>
