...
import { Star, Calendar, Clock, CheckCircle, ArrowRight, Info, ChevronDown, Ticket, PartyPopper, Euro } from 'lucide-react';
...
export function ParkActivityModal({ activity, park, allActivities, onClose, onNavigate }: Props) {
  const [activeTab, setActiveTab] = useState<'infos' | 'tarifs' | 'inclus' | 'avis' | 'faq'>('infos');
  ...
  // Tarifs (déjà fusionnés côté parent : spécifiques au parc ou globaux)
  const tarifs = activity.tarifs;
        ...
        <div className="flex flex-col lg:flex-row flex-1 min-h-0">
          ...
          <div className="flex-1 flex flex-col min-h-0">
            ...
            <div className="flex-1 overflow-y-auto p-6">
              {/* Onglet INFOS avec aperçu des tarifs */}
              {activeTab === 'infos' && (
                  ...
                  {tarifs && tarifs.length > 0 ? (
                    <div className="rounded-2xl overflow-hidden border border-gray-100">
                      <div className="px-4 py-3 flex items-center gap-2" style={{ background: `linear-gradient(to right, ${GREEN}15, ${GREEN}05)` }}>
                        <Euro className="size-4" style={{ color: GREEN }} />
                        <span className="font-black text-sm" style={{ color: GREEN }}>Tarifs</span>
                      </div>
                      <div className="divide-y divide-gray-50">
                        {tarifs.slice(0, 2).map((groupe: any, gi: number) => (
                          <div key={gi} className="p-4">
                            <div className="text-xs font-bold uppercase mb-2" style={{ color: GREEN }}>{groupe.titre}</div>
                            <div className="space-y-2">
                              {groupe.lignes.slice(0, 3).map((ligne: any, li: number) => (
                      ...
                      <div className="px-4 pb-4">
                        <Link
                          ...
                          className="flex items-center justify-center gap-2 w-full py-3 rounded-xl text-white text-xs font-bold transition-all hover:shadow-lg"
                    ...
                    <div className="rounded-2xl bg-gray-50 p-6 text-center border border-gray-100">
                      <p className="text-gray-500 text-sm">Consultez la billetterie pour voir les tarifs</p>
                      <Link
                        to="/booking"
                        state={{ parkId: park.id, activityId: activity.id }}
                        className="inline-flex items-center gap-2 mt-3 px-4 py-2 rounded-full text-white text-xs font-bold"
                        style={{ backgroundColor: GREEN }}
                        onClick={handleClose}
                      >
                        Accéder à la billetterie <ArrowRight className="size-3" />
                      </Link>
                    </div>
                  )}
                ...
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
                  ...
                  {tarifs && tarifs.length > 0 ? (
                    <div className="space-y-4">
                      {tarifs.map((groupe: any, gi: number) => (
                        <div key={gi} className="rounded-2xl border border-gray-100 overflow-hidden">
                          <div className="px-4 py-3 font-black text-sm" style={{ backgroundColor: `${GREEN}10`, color: GREEN }}>
                            {groupe.titre}
                          </div>
                          <div className="divide-y divide-gray-50">
                            {groupe.lignes.map((ligne: any, li: number) => (
                              <div key={li} className="flex items-center justify-between px-4 py-3 hover:bg-gray-50/50">
                                <span className="text-gray-700 font-medium">{ligne.label}</span>
                                <div className="flex items-center gap-3">
                                  <span className="font-black" style={{ color: GREEN }}>{ligne.prix}</span>
                                  <Link
                                    to="/booking"
                                    state={{ parkId: park.id, activityId: activity.id }}
                                    className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg text-white text-xs font-bold transition-all hover:shadow-md"
                                    style={{ backgroundColor: ORANGE }}
                                    onClick={handleClose}
                                  >
                                    Réserver <ArrowRight className="size-2.5" />
                                  </Link>
                                </div>
                              </div>
                            ))}
                    ...
                    <div className="text-center py-8 bg-gray-50 rounded-2xl">
                      <p className="text-gray-500 text-sm mb-3">Aucun tarif spécifique n'est disponible pour cette activité.</p>
                      <Link
                        to="/booking"
                        state={{ parkId: park.id, activityId: activity.id }}
                        className="inline-flex items-center gap-2 px-4 py-2 rounded-full text-white text-xs font-bold"
                        style={{ backgroundColor: GREEN }}
                        onClick={handleClose}
                      >
                        Voir la billetterie <ArrowRight className="size-3" />
                      </Link>
                    </div>
                  )}
                </motion.div>
