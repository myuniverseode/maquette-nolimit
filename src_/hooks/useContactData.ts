...
export function useContactData() {
  ...
  useEffect(() => {
    fetch(`${API_URL}/contact`, {
      ...
      .then(res => { if (!res.ok) throw new Error('Erreur API Contact'); return res.json().then(_x=>cleanWpData(_x)); }).then(d => cleanWpData(d)).then(d => cleanWpData(d))
      .then(_raw36 => { const wpData = cleanWpData(_raw36);
