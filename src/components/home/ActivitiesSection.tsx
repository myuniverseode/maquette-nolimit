        ...
        <div className="container mx-auto px-4 relative z-10">
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="text-center mb-16">
            ...
            <p className="text-white/70 max-w-2xl mx-auto text-lg">
              {isTouch ? "Appuyez une fois pour voir les tarifs, deux fois pour découvrir l'activité" : "Passez la souris sur une activité pour découvrir les tarifs"}
            </p>
          </motion.div>
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-8 max-w-5xl mx-auto justify-items-center">
              {displayedActivities.map((activity, index) => {
                // Tarifs issus directement de l'objet activité (WordPress ou fallback du hook)
                const tarifs: TarifGroupe[] = (activity as any).tarifs ?? [];
...
const ActivityRond = React.forwardRef<HTMLDivElement, {
  activity: any; tarifs: TarifGroupe[]; index: number;
  isExpanded: boolean; isTouch: boolean;
  onExpand: () => void; onCollapse: () => void;
  onClick: (e: React.MouseEvent | React.TouchEvent) => void;
  onSatelliteClick: () => void;
}>(({ activity, tarifs, index, isExpanded, isTouch, onExpand, onCollapse, onClick, onSatelliteClick }, ref) => {
  const hasTarifs = tarifs.length > 0 && tarifs[0]?.lignes?.length > 0;
  return (
    <motion.div ref={ref} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.08, duration: 0.5 }} className="relative" style={{ zIndex: isExpanded ? 50 : 1, width: '200px', height: '200px' }} onMouseEnter={() => !isTouch && hasTarifs && onExpand()} onMouseLeave={() => !isTouch && onCollapse()} onClick={onClick}>

      {/* Rond */}
      <motion.div className="relative w-full h-full rounded-full overflow-hidden border-2 border-white/20 cursor-pointer" style={{ boxShadow: isExpanded ? `0 0 40px ${GREEN}40` : 'none' }} animate={isExpanded ? { scale: 1.1 } : { scale: 1 }} transition={{ duration: 0.3, ease: 'easeOut' }}>
        ...
        <div className="relative h-full flex flex-col items-center justify-center text-center px-4 gap-2">
          ...
          <div className="flex flex-wrap justify-center gap-1.5 mt-1">
            {activity.minAge && <span className="px-2 py-0.5 rounded-full text-[10px] font-bold text-white bg-white/20 border border-white/30">{activity.minAge}+ ans</span>}
            {activity.duration && <span className="px-2 py-0.5 rounded-full text-[10px] font-bold text-white border border-white/30" style={{ backgroundColor: `${ORANGE}50` }}>{activity.duration}</span>}
          </div>
          {isTouch && hasTarifs && <div className="absolute bottom-2 right-2 w-6 h-6 rounded-full bg-white/30 flex items-center justify-center"><ChevronRight className="size-3 text-white" /></div>}
        </div>
      </motion.div>

      {/* Bulles satellites */}
      <AnimatePresence>
        {isExpanded && hasTarifs && (
          <>
            {/* Carte tarifs (haut) */}
            <motion.div initial={{ opacity: 0, y: 10, scale: 0.85 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 10, scale: 0.85 }} transition={{ duration: 0.2 }} className="absolute -top-48 left-1/2 -translate-x-1/2 bg-white rounded-3xl shadow-2xl w-72 overflow-hidden cursor-pointer" style={{ border: `3px solid ${GREEN}`, boxShadow: '0 20px 40px rgba(0,0,0,0.5)' }} onClick={(e) => { e.stopPropagation(); onSatelliteClick(); }} onMouseEnter={() => !isTouch && onExpand()}>
              ...
              <div className="p-4 space-y-3">
                {tarifs.slice(0, 2).map((groupe, gi) => (
                  <div key={gi}>
                    <div className="text-xs font-bold uppercase mb-1.5 tracking-wide" style={{ color: GREEN }}>{groupe.titre}</div>
                ...
                {tarifs.length > 2 && <div className="text-center text-xs font-bold text-gray-400">+{tarifs.length - 2} autres grilles tarifaires</div>}
              </div>
              <div className="px-4 pb-4">
                <div className="flex items-center justify-center gap-2 w-full py-2.5 rounded-xl text-white text-xs font-bold" style={{ backgroundColor: GREEN }}>
            ...
            {/* Durée (gauche) */}
            {activity.duration && (
              <motion.div initial={{ opacity: 0, x: 10, scale: 0.8 }} animate={{ opacity: 1, x: 0, scale: 1 }} exit={{ opacity: 0, x: 10, scale: 0.8 }} transition={{ duration: 0.2, delay: 0.05 }} className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-28 bg-white rounded-2xl shadow-xl px-4 py-3 cursor-pointer" style={{ border: `3px solid ${ORANGE}`, minWidth: '100px' }} onClick={(e) => { e.stopPropagation(); onSatelliteClick(); }}>
            ...
            {/* Âge (droite) */}
            {activity.minAge && (
              <motion.div initial={{ opacity: 0, x: -10, scale: 0.8 }} animate={{ opacity: 1, x: 0, scale: 1 }} exit={{ opacity: 0, x: -10, scale: 0.8 }} transition={{ duration: 0.2, delay: 0.1 }} className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-28 bg-white rounded-2xl shadow-xl px-4 py-3 cursor-pointer" style={{ border: `3px solid ${GREEN}`, minWidth: '100px' }} onClick={(e) => { e.stopPropagation(); onSatelliteClick(); }}>
                <div className="text-center">
            ...
            {/* Badge mobile */}
            {isTouch && (
              <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.8 }} transition={{ duration: 0.2, delay: 0.15 }} className="absolute -bottom-12 left-1/2 -translate-x-1/2 bg-white/90 backdrop-blur-sm rounded-full shadow-lg px-4 py-2 cursor-pointer whitespace-nowrap" style={{ border: `2px solid ${GREEN}` }} onClick={(e) => { e.stopPropagation(); onSatelliteClick(); }}>
                <div className="flex items-center gap-2 text-xs font-black" style={{ color: GREEN }}><span>👆</span><span>Appuyez pour voir l'activité</span></div>
              </motion.div>
            )}
          </>
        )}
