...
// Direction : chaleureux, inclusif, nature organique, récits familiaux
import { Link } from 'react-router-dom';
import { motion } from 'framer-motion';
import {
  Users, Calendar, Phone, ArrowRight, CheckCircle,
  Star, Heart, ChevronRight, TreePine, Sun, MapPin,
  Smile, Baby, Clock, Shield, Camera, Utensils
} from 'lucide-react';
...
const FAMILY_PACKS = [
  {
    ...
    popular: true,
    includes: [
      'Activités multi-niveaux en parallèle',
      'Zone pique-nique réservée',
      'Animateur famille dédié',
  ...
  },
  {
    ...
    desc: 'Un programme pensé pour que chaque génération vive sa propre aventure tout en partageant les mêmes souvenirs. La magie des familles nombreuses.',
    minPeople: 12,
    maxPeople: 80,
    price: 'Sur devis',
    color: TEAL,
    popular: false,
    includes: [
      'Parcours seniors adaptés PMR',
      'Zone jeux libres pour les tout-petits',
      'Activités adultes et ados séparées',
      'Point de rendez-vous commun',
      'Repas / pique-nique coordonné',
    ],
  },
  {
    id: 'weekend',
...
function FamilleHero() {
  return (
    <section className="relative min-h-[85vh] overflow-hidden flex items-end">
      <div className="absolute inset-0">
        <img
          src="https://images.unsplash.com/photo-1511895426328-dc8714191300?w=1800&h=900&fit=crop"
          alt="Grande famille en aventure"
      ...
      <div className="relative z-10 container mx-auto px-6 pb-24 w-full">
        ...
        <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.9, ease: [0.16, 1, 0.3, 1] }}>
          ...
          <h1 className="text-6xl md:text-8xl font-black text-white leading-[0.9] tracking-tight mb-6">
            Des moments<br />
            qui durent<br />
            <span style={{ color: '#86efac' }}>toujours.</span>
          </h1>

          <p className="text-white/70 text-xl max-w-xl leading-relaxed mb-10">
            Des grands-parents aux petits-enfants — un endroit où toutes les générations se retrouvent autour d'une même aventure, chacun à son rythme.
          </p>
          ...
          <div className="flex flex-wrap gap-3 mt-10">
            {[
              { icon: '👶', text: 'Dès 0 an' },
              { icon: '👴', text: "Jusqu'à 90 ans" },
              { icon: '♿', text: 'Accessible PMR' },
              { icon: '🆓', text: '-3 ans gratuit' },
            ].map(({ icon, text }) => (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 max-w-6xl mx-auto">
          {GENERATIONS.map((gen, i) => (
            <motion.div
              ...
              style={{ borderColor: `${gen.color}35` }}
            >
              ...
              >
                ...
                <div className="p-5 border-t border-gray-100">
                  <Link
                    ...
                    style={pack.popular
                      ? { background: `linear-gradient(to right, ${GREEN}, #4a9d00)`, color: 'white' }
                      : { backgroundColor: `${pack.color}10`, color: pack.color }
                    }
                  >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl mx-auto">
          {TESTIMONIALS_FAMILY.map((t, i) => (
            <motion.div
              ...
              transition={{ delay: i * 0.1 }}
              whileHover={{ y: -5 }}
              className="rounded-3xl p-7 bg-white/5 border border-white/10 backdrop-blur-sm"
            >
    ...
    <section className="py-20 bg-gray-50">
      <div className="container mx-auto px-6 max-w-4xl text-center">
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
          ...
          <h2 className="text-4xl font-black mb-4" style={{ color: DARK }}>
            Votre prochaine réunion de famille<br />
            <span style={{ color: GREEN }}>commence ici</span>
          </h2>
