// pages/PourQuiDuoPage.tsx — Pour les duos (dynamique via API)
import { motion, useScroll, useTransform } from 'framer-motion';
import { ArrowRight, Heart, Star, Calendar, Zap, ChevronDown } from 'lucide-react';
...
// Fallbacks utilisés UNIQUEMENT si l'API ne retourne rien
const FALLBACK_ACTIVITIES = [
  { emoji: '🧗', name: 'Accrobranche',     desc: 'Encouragez-vous mutuellement dans les hauteurs.',  diff: 'Tous niveaux', price: 24 },
  { emoji: '🏹', name: 'Tir à l\'arc',     desc: 'Défi amical ou duo synchronisé.', diff: 'Débutant',    price: 18 },
  { emoji: '🪂', name: 'Tyrolienne',        desc: 'Le frisson partagé d\'une descente côte à côte.',   diff: 'Intermédiaire', price: 29 },
  { emoji: '🧩', name: 'Escape game forêt', desc: 'Résolvez les énigmes ensemble.', diff: 'Tous niveaux', price: 34 },
  { emoji: '🛶', name: 'Canoë duo',         desc: 'Trouvez votre rythme sur l\'eau.',         diff: 'Débutant',    price: 22 },
  { emoji: '⛰️', name: 'Via ferrata',       desc: 'Un défi vertical qui rapproche.',          diff: 'Intermédiaire', price: 38 },
];
...
const FALLBACK_TESTIMONIALS = [
  { text: "On cherchait quelque chose d'original pour nos 5 ans. La tyrolienne en duo, c'était exactement ça.", author: "Camille & Théo", stars: 5 },
  { text: "Mon copain avait peur des hauteurs. Après l'accrobranche, il voulait recommencer.", author: "Inès", stars: 5 },
...
export function PourQuiDuoPage() {
  ...
  // ── Données depuis l'API, avec fallbacks ──
  const hero = duoData.hero || { title: "À deux, tout change.", subtitle: "Parce que les meilleurs souvenirs se créent avec quelqu'un.", badge: 'Pour les duos' };
  const intro = duoData.intro || { title: "L'aventure en duo, c'est magique", text: "" };
  const moments = duoData.moments?.length > 0 ? duoData.moments : FALLBACK_MOMENTS;
  const pack = duoData.pack || { title: 'Pack Duo', discount: '-15%', description: '', features: [], stats: [] };
  const cta = duoData.cta || { title: "Prêts pour l'aventure ?", subtitle: '' };
  const blogSection = duoData.blog || { title: 'Idées & actualités', subtitle: '' };
  const blogPosts = duoData.blogPosts || [];

  // Activités : si l'admin a défini des activités duo spécifiques, on les utilise.
  // Sinon, on utilise les vraies activités WordPress (realActivities).
  // En dernier recours, fallback hardcodé.
  const duoActivities = duoData.activities?.length > 0
    ? duoData.activities
    : duoData.realActivities?.length > 0
      ? duoData.realActivities.map((a: any) => ({
          emoji: a.emoji, name: a.name, desc: a.description, diff: a.difficulty, price: null,
        }))
      : FALLBACK_ACTIVITIES;

  // Témoignages : l'API renvoie soit les custom, soit les vrais avis tagués
  const testimonials = duoData.testimonials?.length > 0
    ? duoData.testimonials.map((t: any) => ({
  ...
  const packFeatures = pack.features?.length > 0 ? pack.features : ['2 places au tarif réduit', 'Même activité, même créneau', 'Photo souvenir offerte', 'Annulation gratuite 48h avant'];
  const packStats = pack.stats?.length > 0 ? pack.stats : [
  ...
  return (
    <div className="min-h-screen bg-white overflow-x-hidden">
      ...
      <section ref={heroRef} className="relative h-screen flex items-end overflow-hidden">
        <motion.div style={{ y: heroY }} className="absolute inset-0 scale-110">
          <div className="w-full h-full" style={{
            background: `radial-gradient(ellipse at 30% 40%, #1a4a0a 0%, transparent 55%), radial-gradient(ellipse at 75% 60%, ${ORANGE}55 0%, transparent 50%), linear-gradient(160deg, #0d2b04 0%, #1e5c08 40%, #2d1a00 100%)`,
          }} />
          {[...Array(12)].map((_, i) => (
            <motion.div key={i} className="absolute rounded-full bg-white/5"
              style={{ width: 20 + i * 15, height: 20 + i * 15, left: `${8 + i * 8}%`, top: `${10 + (i % 4) * 20}%` }}
        ...
        <motion.div style={{ opacity: heroOpacity }} className="relative z-20 container mx-auto px-6 pb-20 md:pb-28">
          ...
          <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.9, ease: 'easeOut' }} className="max-w-3xl">
            ...
            <h1 className="text-6xl md:text-8xl font-black text-white leading-none mb-6" dangerouslySetInnerHTML={{
              __html: hero.title.includes(',')
                ? hero.title.replace(/,(.*)$/, ',<br /><span style="color:' + ORANGE + '">$1</span>')
                : hero.title
            }} />
            ...
            <div className="flex flex-wrap gap-4">
              <Link to="/parcs" className="inline-flex items-center gap-2 px-7 py-4 rounded-2xl font-black text-white text-base"
              ...
              <Link to="/activities" className="inline-flex items-center gap-2 px-7 py-4 rounded-2xl font-bold text-white border-2 border-white/25 hover:border-white/50 hover:bg-white/10 transition-all">
                Voir les activités
              </Link>
            </div>
          </motion.div>
        ...
        <div className="container mx-auto px-6 max-w-4xl text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            <h2 className="text-3xl md:text-4xl font-black mb-4" style={{ color: DARK }}>
              {intro.title ? intro.title.replace(/(\S+)$/, `<span style="color:${ORANGE}">$1</span>`) : ''}
            </h2>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-5 max-w-4xl mx-auto">
            {moments.map((m: any, i: number) => (
              <motion.div key={i} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
                transition={{ delay: i * 0.09 }} whileHover={{ y: -6, scale: 1.03 }}
      ...
      <section className="py-20" style={{ backgroundColor: DARK }}>
        <div className="container mx-auto px-6">
          <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-14">
            ...
            <h2 className="text-4xl md:text-5xl font-black text-white mb-3">Fait pour <span style={{ color: ORANGE }}>deux.</span></h2>
            <p className="text-white/50">Sélection d'activités qui prennent tout leur sens en duo.</p>
          </motion.div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 max-w-6xl mx-auto">
            {duoActivities.map((act: any, i: number) => (
              <motion.div key={i} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
                transition={{ delay: i * 0.08 }} whileHover={{ y: -8, scale: 1.02 }}
                ...
                <div className="flex items-start justify-between gap-3 mb-3">
                  ...
                  {act.price && <div className="shrink-0 text-2xl font-black" style={{ color: ORANGE }}>{act.price}€</div>}
                </div>
                <p className="text-white/50 text-sm leading-relaxed mb-4">{act.desc || act.description}</p>
                <div className="flex items-center justify-between">
                  <span className="px-3 py-1 rounded-lg text-xs font-bold" style={{ backgroundColor: `${GREEN}25`, color: '#7dda30' }}>
                    {act.diff || act.difficulty}
                  </span>
                  <motion.div initial={{ x: 0 }} whileHover={{ x: 4 }}
                    className="flex items-center gap-1 text-xs font-bold opacity-0 group-hover:opacity-100 transition-opacity" style={{ color: ORANGE }}>
          ...
          <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
            ...
            <div className="p-10 md:p-14 grid md:grid-cols-2 gap-10 items-center">
              <div>
                ...
                <h2 className="text-4xl font-black text-white mb-4">{pack.title}<br /><span style={{ color: ORANGE }}>{pack.discount}</span></h2>
                <p className="text-white/70 leading-relaxed mb-6">{pack.description || "Réservez 2 places sur la même activité et profitez d'une réduction immédiate."}</p>
                <ul className="space-y-2 mb-8">
                  {packFeatures.map((item: string, i: number) => (
              ...
              <div className="grid grid-cols-2 gap-4">
                {packStats.map(({ val, label }: any, i: number) => (
                  <motion.div key={i} whileHover={{ scale: 1.05 }} className="text-center rounded-2xl p-5"
            ...
            {testimonials.map((t: any, i: number) => (
              <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
                ...
                <div className="flex gap-1 mb-4">
                  {[...Array(t.stars || t.rating || 5)].map((_, s) => (
      ...
      {/* ── SECTION BLOG (vrais articles) ── */}
      {blogPosts.length > 0 && (
        <section className="py-20 bg-white border-t border-gray-100">
          <div className="container mx-auto px-6">
            <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-12">
              <h2 className="text-4xl font-black mb-3" style={{ color: DARK }}>
                {blogSection.title?.split(' ').slice(0, -1).join(' ')}{' '}
                <span style={{ color: ORANGE }}>{blogSection.title?.split(' ').slice(-1)}</span>
              </h2>
              ...
              {blogPosts.map((post: any, i: number) => (
                <motion.div key={post.id || i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
                  ...
                  <div className="p-6">
                    ...
                    <Link to={`/blog/${post.slug}`} className="inline-flex items-center gap-1 text-sm font-bold transition-colors" style={{ color: ORANGE }}>
        ...
        <div className="container mx-auto px-6 text-center max-w-3xl">
          <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            ...
            <h2 className="text-4xl md:text-5xl font-black mb-4" style={{ color: DARK }}>
              {cta.title?.includes('?')
                ? <>{cta.title.split('?')[0]}?<br /><span style={{ color: ORANGE }}>{'?'}</span></>
                : cta.title
              }
            </h2>
            <p className="text-gray-500 mb-8">{cta.subtitle}</p>
