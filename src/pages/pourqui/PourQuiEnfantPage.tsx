// pages/PourQuiEnfantPage.tsx — Pour les enfants (dynamique via API)
...
// Fallbacks UNIQUEMENT si l'API ne retourne rien
const FALLBACK_CHILD_ACTIVITIES = [
  { emoji: '🌳', name: 'Parcours Découverte', ageMin: 5, duration: '1h30', color: GREEN, bg: '#f0fdf4', desc: 'Premier contact avec les arbres.', badges: ['Dès 5 ans', 'Encadré', 'Matériel adapté'] },
  { emoji: '🏹', name: 'Tir à l\'Arc Juniors', ageMin: 7, duration: '1h', color: ORANGE, bg: '#fef3ea', desc: 'Concentration et précision.', badges: ['Dès 7 ans', 'Arcs adaptés', 'Médaille'] },
  { emoji: '🧗', name: 'Accrob\'Junior', ageMin: 8, duration: '2h', color: YELLOW, bg: '#fffbeb', desc: 'Le grand parcours accrobranche.', badges: ['8–14 ans', 'Moniteur dédié', 'Certificat'] },
  { emoji: '🔍', name: 'Chasse au Trésor', ageMin: 5, duration: '1h30', color: PURPLE, bg: '#f5f3ff', desc: 'Aventure en forêt avec énigmes.', badges: ['Dès 5 ans', 'Groupes', 'Goûter offert'] },
  { emoji: '🪁', name: 'Tyrolienne Enfant', ageMin: 6, duration: '45 min', color: PINK, bg: '#fdf2f8', desc: 'Un moment de vertige contrôlé.', badges: ['Dès 6 ans', 'Max 40 kg', 'Moniteur'] },
  { emoji: '🌿', name: 'Atelier Nature', ageMin: 4, duration: '1h', color: GREEN, bg: '#f0fdf4', desc: 'Reconnaissance des plantes et insectes.', badges: ['Dès 4 ans', 'Petit groupe', 'Carnet offert'] },
];
...
const FALLBACK_TESTIMONIALS = [
  { text: "Ma fille de 7 ans avait peur au départ. Au bout de 10 minutes, elle ne voulait plus descendre.", author: "Sophie L., maman", stars: 5 },
  { text: "L'atelier nature a rendu mes fils curieux de tout. Depuis, ils identifient les plantes.", author: "Marc D., papa", stars: 5 },
  { text: "La chasse au trésor était parfaite pour l'anniversaire. 12 enfants comblés.", author: "Aline K.", stars: 5 },
];

const FALLBACK_BIRTHDAY_FEATURES = [
...
export function PourQuiEnfantPage() {
  ...
  const hero = enfantData.hero || { title: "Leur première grande aventure.", subtitle: "Des activités pensées pour chaque âge.", badge: 'Pour les enfants' };
  const intro = enfantData.intro || { title: "Pour les enfants en famille", text: "" };
  const safetyPoints = enfantData.safetyPoints?.length > 0 ? enfantData.safetyPoints : FALLBACK_SAFETY;
  const birthday = enfantData.birthday || { title: 'Anniversaire inoubliable.', description: '', price: 'À partir de 199€', capacity: "jusqu'à 12 enfants", features: [] };
  const birthdayFeatures = birthday.features?.length > 0 ? birthday.features : FALLBACK_BIRTHDAY_FEATURES;
  const ctaData = enfantData.cta || { title: "Leur prochaine aventure commence ici.", subtitle: "Réservez en 2 minutes." };

  // Activités : custom admin > vraies activités WP > fallback
  const childActivities = enfantData.activities?.length > 0
    ? enfantData.activities
    : enfantData.realActivities?.length > 0
      ? enfantData.realActivities.map((a: any) => ({
          emoji: a.emoji, name: a.name, ageMin: a.minAge, duration: a.duration,
          color: GREEN, bg: '#f0fdf4', desc: a.description,
          badges: [`Dès ${a.minAge} ans`, a.difficulty],
        }))
      : FALLBACK_CHILD_ACTIVITIES;

  // Témoignages : API (custom ou vrais avis tagués) > fallback
  const testimonials = enfantData.testimonials?.length > 0
    ? enfantData.testimonials.map((t: any) => ({
      ...
      <section className="relative h-screen flex items-end overflow-hidden">
        <motion.div style={{ y: heroY }} className="absolute inset-0 scale-110">
          <div className="w-full h-full" style={{
            background: `radial-gradient(ellipse at 15% 40%, ${YELLOW}50 0%, transparent 40%), radial-gradient(ellipse at 80% 25%, ${PURPLE}40 0%, transparent 40%), radial-gradient(ellipse at 55% 70%, ${GREEN}60 0%, transparent 50%), linear-gradient(160deg, #0d2b04 0%, #1a5c08 40%, #0f1a0a 100%)`,
          }} />
          {['⭐', '🌟', '✨', '🍃', '🌿', '🦋', '🐞', '🌸'].map((star, i) => (
            <motion.div key={i} className="absolute text-2xl select-none pointer-events-none"
              style={{ left: `${8 + i * 12}%`, top: `${12 + (i % 3) * 22}%` }}
              animate={{ y: [0, -15, 0], rotate: [0, 10, -10, 0], opacity: [0.4, 0.8, 0.4] }}
              transition={{ duration: 4 + i * 0.5, repeat: Infinity, delay: i * 0.3 }}>{star}</motion.div>
          ))}
        </motion.div>
        ...
        <motion.div style={{ opacity: heroOpacity }} className="relative z-20 container mx-auto px-6 pb-20 md:pb-28">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="flex items-center gap-2 mb-8">
            ...
            <span className="text-white/30">/</span><span className="text-white text-sm font-bold">Enfant</span>
          </motion.div>
          <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.9 }} className="max-w-3xl">
            ...
            <h1 className="text-6xl md:text-8xl font-black text-white leading-none mb-6" dangerouslySetInnerHTML={{
              __html: hero.title.includes('\n')
                ? hero.title.replace(/\n(.*)$/, '<br /><span style="color:' + YELLOW + '">$1</span>')
                : hero.title
            }} />
        ...
        <div className="container mx-auto px-6">
          <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-14">
            <h2 className="text-4xl md:text-5xl font-black mb-3" style={{ color: DARK }}>
              {childActivities.length} activités <span style={{ color: GREEN }}>rien que pour eux.</span>
            </h2>
            <p className="text-gray-500">Progressives, encadrées, mémorables.</p>
          </motion.div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-6xl mx-auto">
            {childActivities.map((act: any, i: number) => (
              <motion.div key={i} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
                transition={{ delay: i * 0.08 }} whileHover={{ y: -8, scale: 1.01 }}
                className="bg-white rounded-3xl overflow-hidden border-2 hover:shadow-xl transition-all"
                style={{ borderColor: `${act.color || GREEN}30` }}>
                <div className="h-24 flex items-center justify-center text-6xl" style={{ backgroundColor: act.bg || '#f0fdf4' }}>
                  {act.emoji}
                </div>
                <div className="p-6">
                  <div className="flex items-start justify-between gap-2 mb-3">
                    <h3 className="font-black text-xl" style={{ color: DARK }}>{act.name}</h3>
                    {act.duration && <span className="text-xs font-bold shrink-0 px-2.5 py-1 rounded-lg" style={{ backgroundColor: `${act.color || GREEN}15`, color: act.color || GREEN }}>
                      {act.duration}
                    </span>}
                  </div>
                  <p className="text-gray-500 text-sm leading-relaxed mb-4">{act.desc || act.description}</p>
                  {act.badges && <div className="flex flex-wrap gap-1.5">
                    {act.badges.map((badge: string, bi: number) => (
                      <span key={bi} className="px-2.5 py-1 rounded-lg text-[11px] font-bold" style={{ backgroundColor: `${act.color || GREEN}12`, color: act.color || GREEN }}>
                        {badge}
                      </span>
                    ))}
                  </div>}
            ...
            <div className="rounded-3xl overflow-hidden border-2" style={{ borderColor: `${YELLOW}60` }}>
              <div className="grid md:grid-cols-2">
                <div className="p-10 md:p-12" style={{ background: `linear-gradient(135deg, ${YELLOW}20, ${ORANGE}10)` }}>
                  ...
                  <p className="text-gray-600 leading-relaxed mb-8">{birthday.description || "Offrez à votre enfant un anniversaire qui sort de l'ordinaire."}</p>
                  <div className="flex items-center gap-4">
                    <div>
                      <div className="text-xs text-gray-400 font-bold uppercase tracking-wider">Pack anniversaire</div>
                      <div className="text-3xl font-black" style={{ color: YELLOW }}>{birthday.price}</div>
                      <div className="text-xs text-gray-400">{birthday.capacity}</div>
                    </div>
                  </div>
                  <Link to="/booking" className="mt-7 inline-flex items-center gap-2 px-7 py-4 rounded-2xl font-black"
                ...
                <div className="p-10 bg-white">
                  ...
                  <ul className="space-y-3">
                    {birthdayFeatures.map((f: string, i: number) => (
      ...
      <section className="py-20" style={{ backgroundColor: DARK }}>
        <div className="container mx-auto px-6">
          <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-14">
            ...
            <h2 className="text-4xl font-black text-white mb-3">Leur sécurité,<br /><span style={{ color: GREEN }}>notre obsession.</span></h2>
            <p className="text-white/50 max-w-xl mx-auto">Chaque détail est pensé pour que les parents puissent souffler.</p>
          </motion.div>
      ...
      <section className="py-16 border-t border-gray-100 bg-white">
        <div className="container mx-auto px-6 text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            ...
            <Link to="/parcs" className="inline-flex items-center gap-2 px-8 py-4 rounded-2xl font-black"
              style={{ background: `linear-gradient(135deg, ${YELLOW}, #f97316)`, color: DARK, boxShadow: `0 8px 28px ${YELLOW}40` }}>
