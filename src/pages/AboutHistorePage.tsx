// pages/AboutHistorePage.tsx — Histoire avec bloc texte au lieu des pictos + sidebar
...
const DEFAULT_TIMELINE = [
  { year: '2015', emoji: '🌲', title: 'Naissance à Chevry',              desc: "Trois amis passionnés ouvrent le premier parc NoLimit.", color: GREEN,  side: 'left'  },
  { year: '2017', emoji: '📍', title: 'Nemours nous appelle',            desc: "Deuxième parc en Seine-et-Marne.",                       color: ORANGE, side: 'right' },
  { year: '2019', emoji: '🏆', title: 'Montargis & la croissance',       desc: "Troisième parc, première équipe de +30 personnes.",      color: GREEN,  side: 'left'  },
  { year: '2021', emoji: '🌿', title: 'Digny : renaissance post-Covid',  desc: "Malgré la tempête, on inaugure Digny.",                  color: ORANGE, side: 'right' },
  { year: '2022', emoji: '🚀', title: "Le Coudray & l'ambition",         desc: "Cinquième parc, premier label Partenaire ONF.",          color: GREEN,  side: 'left'  },
  { year: '2025', emoji: '✨', title: "Aujourd'hui & demain",             desc: "5 parcs, 75 000+ aventuriers, cap sur 2027.",            color: ORANGE, side: 'right' },
];
...
export function AboutHistorePage() {
  ...
  const timeline = data.timeline?.length > 0
    ? data.timeline.map((item, i) => ({
        year: item.year, emoji: item.emoji ?? item.icon ?? '📍', title: item.title,
        desc: item.desc, color: item.color ?? (i % 2 === 0 ? GREEN : ORANGE),
  ...
  // Texte de présentation qui remplace les 4 pictos
  const introTitle = (data as any).introTitle || "L'histoire de NoLimit Aventure";
  const introText = (data as any).introText || "Depuis 2015, NoLimit Aventure écrit son histoire une saison après l'autre. De trois amis passionnés qui ont planté leurs premiers parcours dans la forêt de Chevry, à un réseau de 5 parcs accueillant plus de 75 000 aventuriers chaque année — voici les étapes clés de cette aventure collective.";
        ...
        <div className="container mx-auto px-6 relative z-10">
          ...
          <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }} className="max-w-3xl">
            ...
            <p className="text-lg text-white/65 leading-relaxed max-w-xl">
              {hero.intro || "De trois amis et une idée folle à un réseau de parcs."}
            </p>
          </motion.div>
        </div>
      ...
      <div className="container mx-auto px-6 py-12">
        <div className="flex flex-col lg:flex-row gap-10">
          ...
          <main className="flex-1 min-w-0">
            ...
            {/* Bloc texte de présentation (remplace les 4 pictos stats) */}
            ...
            <section className="py-16">
              <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-14">
                ...
                <p className="text-gray-500">{sections.timelineSubtitle || 'Une étape à la fois.'}</p>
              </motion.div>
            ...
            {/* VISION */}
            <section className="py-16 rounded-3xl overflow-hidden" style={{ backgroundColor: DARK }}>
                ...
                <div className="grid md:grid-cols-2 gap-10 items-center">
                  <div>
                    ...
                    <h2 className="text-3xl md:text-4xl font-black text-white mb-5">
                      {sections.visionTitle || <>Leader national du <span style={{ color: ORANGE }}>parc aventure.</span></>}
                    </h2>
                    <p className="text-white/65 leading-relaxed mb-8">
                      {sections.visionSubtitle || data.vision || "Notre objectif est clair : créer le réseau de parcs le plus qualitatif de France."}
                    </p>
